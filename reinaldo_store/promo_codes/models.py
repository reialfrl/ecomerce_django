...
class PromoCodeManager(models.Manager):
    def get_valid(self, code):
        #Función para validar el codigo de promoción según la duración ingresada al crear el cupos y se uso. 
        now = timezone.now() 
        return self.filter(code=code).filter(used=False).filter(valid_from__lte=now).filter(valid_to__gte=now).first()

class PromoCode(models.Model):
    #Clase para crear el formulario del cupón.
...
def set_code(sender, instance, *args, **kwargs):
    #Función para crear el codigo del cupón.
    if instance.code:
        return
