...
class BillingProfileManager(models.Manager):
    #Clase para crear un token en stripe en base el carrito de compra creado por el usuario.
    def create_by_stripe_token(self, user, stripe_token):
        if user.has_customer() and stripe_token:
            ...
            return self.create(card_id=source.id, last4=source.last4, token=stripe_token, brand=source.brand, 
                                user=user, default=not user.has_billing_profiles())

class BillingProfile(models.Model):
    #Clase para crear el formulario del metodo de pago.
